...
public class C24291_VerifyCourtesyPayOption extends GenericKeywords {
	ExtentReports extent;
	ExtentTest test;

	/*
	 * As a non-member applicant, when I don't select the Courtesy Pay option
	 * the Courtesy Pay information on the Account Agreements and Disclosure
	 * page is not displayed. Created Date : 03-18-2020 Created By : Ashish
	...
	@Test
	public void C24291_VerifyCourtesyPayOption() throws InterruptedException, MessagingException, IOException {
		...
		if (continuetestcase == true) {
			...
			for (startIter = 1; startIter <= totalRowCount; startIter++) {
				if (this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter))) {
					...
					test.log(Status.INFO, "Classic Checking with courtesy pay not selected");
					// Applicant Info
					...
					test.log(Status.PASS, "The 'Accounts Agreements and Disclosures' page is displayed.");

					verifyElementNotPresent(ObjectRepository.courtesyPayTtl);
					test.log(Status.PASS, "Courtesty Pay info not displayed");
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS,
					"As a non-member applicant, when I don't select the Courtesy Pay option the Courtesy Pay information on the Account Agreements and Disclosure page is not displayed.");
		}
	}
