...
public class C23962_NonMemberUserShouldEnterAllMandatoryDetailsInContactInformationSection extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify that Non member user Should enter all mandatory field details in contact information section
	 */
	...
	@Test
	public void C23962_NonMemberUserShouldEnterAllMandatoryDetailsInContactInformationSection() throws InterruptedException, MessagingException, IOException
	{
...
//					test.log(Status.INFO, "Name suffix entered");
					...
					//Deleteing the value from phone number field and click on continue then check
					getElement(ObjectRepository.phonenmbr_txt).clear();
					test.log(Status.INFO, "Phone number deleted");
					getElement(ObjectRepository.continue_further).click();
					verifyText(ObjectRepository.errorMsg1,"Please Enter a Phone Number.");
					if(getElement(ObjectRepository.phonenmbr_txt).getAttribute("style").contains("lightyellow")){
						Assert.assertTrue(true, "Phone Number filed got highlighted with Yellow background");
					}
					
					//Entering a valid value in Phone Number field and procced
					getElement(ObjectRepository.phonenmbr_txt).sendKeys(phonenmbr);
					test.log(Status.INFO, "Phone number entered");
					getElement(ObjectRepository.continue_further).click();
					verifyElementPresent(ObjectRepository.memberShipEligibilityPage);
					...
					//Keeping Phone Type field as Blank and check
					selectDropdownOptContain(ObjectRepository.phonetyp_drop, "Phone Type");
					test.log(Status.INFO, "Phone type selected");
					getElement(ObjectRepository.continue_further).click();
					verifyText(ObjectRepository.errorMsg1,"Please select a phone type.");
					
					//Enter valid Phone Type and procced
					selectDropdownOptContain(ObjectRepository.phonetyp_drop, phonetyp);
					test.log(Status.INFO, "Phone type selected");
					getElement(ObjectRepository.continue_further).click();
					verifyElementPresent(ObjectRepository.memberShipEligibilityPage);
					...
					//Deleting Email address from the field and check
					getElement(ObjectRepository.primaryemail_txt).clear();
					getElement(ObjectRepository.continue_further).click();
					verifyText(ObjectRepository.errorMsg1,"The email address cannot be verified. Please check and retry.");
					
					
					//Entering valid email address and check
					getElement(ObjectRepository.primaryemail_txt).clear();
					getElement(ObjectRepository.primaryemail_txt).sendKeys(primary_email);
					getElement(ObjectRepository.continue_further).click();
					verifyElementPresent(ObjectRepository.memberShipEligibilityPage);
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify that Non member user should get alert for invalid email address entry");
		}
	}
