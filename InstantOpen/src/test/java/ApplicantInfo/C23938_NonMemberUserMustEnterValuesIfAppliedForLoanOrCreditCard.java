...
public class C23938_NonMemberUserMustEnterValuesIfAppliedForLoanOrCreditCard extends GenericKeywords  {
	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify Non member user must enter values if they applied for loan or credit card
	 */
	@BeforeTest
	...
	@Test
	public void C23938_NonMemberUserMustEnterValuesIfAppliedForLoanOrCreditCard() throws InterruptedException, MessagingException, IOException
	{
...
//					test.log(Status.INFO, "Name suffix entered");
					...
					//Verify Loan Information section
					String allLoanInfoFields[] = new String[]{ObjectRepository.housePymt,ObjectRepository.houseType,ObjectRepository.addrYears,
							ObjectRepository.addrMonths,ObjectRepository.addrMonths,ObjectRepository.empStatus,
							ObjectRepository.monIncome,ObjectRepository.empYears,ObjectRepository.empMonths,ObjectRepository.prevEmp,
							ObjectRepository.prevIncome,ObjectRepository.prevYears};
					
							for(String eachField :allLoanInfoFields ){
								verifyElementPresent(eachField);
							}
								
					//Houing Type Field
					List<WebElement>housingType = retrunElements("//select[@id='HousingType_DropDownList']//option");
					for(WebElement eachOption:housingType){
						String option = eachOption.getAttribute("selected");
						if(option != null){
							String defaultOption = eachOption.getText();
							Assert.assertEquals("Own", defaultOption);
						}
					}
				
					selectDropdownOpt(ObjectRepository.houseType,"Mortgage");
					getElement(ObjectRepository.housePymt).sendKeys("1000");
					getElement(ObjectRepository.addrYears).sendKeys("18");
					getElement(ObjectRepository.addrMonths).sendKeys("6");
					selectDropdownOpt(ObjectRepository.empStatus,"Part Time");
					getElement(ObjectRepository.monIncome).sendKeys("25000");
					getElement(ObjectRepository.empYears).sendKeys("19");
					getElement(ObjectRepository.empMonths).sendKeys("10");
					getElement(ObjectRepository.prevEmp).sendKeys("Test");
					getElement(ObjectRepository.prevIncome).sendKeys("15000");
					getElement(ObjectRepository.prevYears).sendKeys("18");
					getElement(ObjectRepository.prevMonths).sendKeys("2");
					
					//Check error message for Housing Payment field
					getElement(ObjectRepository.housePymt).clear();
					getElement(ObjectRepository.continue_btn).click();
					verifyText(ObjectRepository.specificErrorSummary,"Please enter an Amount.");
					
					//Check error message for lived in address field 
					getElement(ObjectRepository.housePymt).sendKeys("1000");
					getElement(ObjectRepository.addrYears).clear();
					getElement(ObjectRepository.addrMonths).clear();
					getElement(ObjectRepository.continue_btn).click();
					verifyText(ObjectRepository.specificErrorSummary,"Please enter how long you have lived at this address in years and months.");
					
					//Check Employee Status field
					
					getElement(ObjectRepository.addrYears).sendKeys("18");
					getElement(ObjectRepository.addrMonths).sendKeys("6");
					selectDropdownOpt(ObjectRepository.empStatus,"Please Select an Item from the drop down list.");
					getElement(ObjectRepository.continue_btn).click();
					verifyText(ObjectRepository.specificErrorSummary,"Please select an item from the drop down list.");
					
					//Check Gross Ammount field 
					selectDropdownOpt(ObjectRepository.empStatus,"Part Time");
					getElement(ObjectRepository.monIncome).clear();
					getElement(ObjectRepository.continue_btn).click();
					verifyText(ObjectRepository.specificErrorSummary,"Please enter an Amount.");
					
					//Employeed years and months
					getElement(ObjectRepository.monIncome).sendKeys("25000");
					getElement(ObjectRepository.empYears).clear();
					getElement(ObjectRepository.empMonths).clear();
					getElement(ObjectRepository.continue_btn).click();
					verifyText(ObjectRepository.specificErrorSummary,"Please enter how long you have been employed in years and months.");
					getElement(ObjectRepository.empYears).sendKeys("19");
					getElement(ObjectRepository.empMonths).sendKeys("10");
					getElement(ObjectRepository.continue_btn).click();
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify Non member user must enter values if they applied for loan or credit card");
		}
	}
