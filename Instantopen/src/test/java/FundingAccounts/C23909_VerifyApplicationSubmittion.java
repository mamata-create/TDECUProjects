...
public class C23909_VerifyApplicationSubmittion   extends GenericKeywords{
  ...
  @Test
  public void C23909_VerifyApplicationSubmittion() throws InterruptedException, MessagingException, IOException {
	  if(continuetestcase==true)
	  {
			...
			//Membership Eligibility page		
					...
					Thread.sleep(2000);
					//getElement(ObjectRepository.livework_btn).click();
...
//					
				...
				//Account Funding
					...
					getElement(ObjectRepository.clscchkng_txt).sendKeys("1000");
					test.log(Status.INFO, "Funding Amount for classic check account entered");
					
					getElement(ObjectRepository.share_txt).sendKeys("1000");
					test.log(Status.INFO, "Funding Amount for Share account entered");
					...
					scrollToElement(ObjectRepository.elctrnchk_opt);
					getElement(ObjectRepository.elctrnchk_opt).click();
					test.log(Status.INFO, "Electronic Check option selected");
					
					Thread.sleep(2000);
					
					getElement(ObjectRepository.elctrnc_rtngnmbr_txt).sendKeys("321456987");
					test.log(Status.INFO, "Routing number entered");
					
					getElement(ObjectRepository.elctrnc_acntnmbr_txt).sendKeys("5060123465");
					test.log(Status.INFO, "Account number entered");
					
					getElement(ObjectRepository.elctrnc_svng_btn).click();
					test.log(Status.INFO, "Savings option selected");
  ...
  public void afterMethod(ITestResult result) throws Throwable {
	  if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify application submittion scenario working fine");
		}
	}
