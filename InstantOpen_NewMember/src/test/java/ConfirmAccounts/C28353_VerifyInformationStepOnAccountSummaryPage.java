...
public class C28353_VerifyInformationStepOnAccountSummaryPage extends GenericKeywords  {
	ExtentReports extent;
	ExtentTest test;
	/*
	 * As an applicant, I expect the Verification step to be highlighted when I am on the Account Summary page.
	 */
	@BeforeTest
	...
	@Test
	public void C28353_VerifyInformationStepOnAccountSummaryPage() throws InterruptedException, MessagingException, IOException
	{
...
//					test.log(Status.INFO, "Name suffix entered");
					...
					for(WebElement eachOption :retrunElements(ObjectRepository.progressBarOptions)){
						...
						if(text_from_option.equalsIgnoreCase("Verification") && class_prop.equals("JourneyBarCurrent")){
							Assert.assertTrue(true,"Verification Step is not currently selected and highlighted on the page" );
							return;
						}else if(text_from_option.equalsIgnoreCase("Information") && class_prop.equals("JourneyBarPrevious")){
							Assert.assertTrue(true,"Information step is not currently selected and highlighted in different blue color");
						}
						index++;
					}
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "As an applicant, I expect the Verification step to be highlighted when I am on the Account Summary page.");
		}
	}
