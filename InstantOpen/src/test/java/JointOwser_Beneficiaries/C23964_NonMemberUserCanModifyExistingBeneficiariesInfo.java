...
public class C23964_NonMemberUserCanModifyExistingBeneficiariesInfo extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify that Non member user can modify existing Beneficiary info
	 */
	...
	@Test
	public void C23964_NonMemberUserCanModifyExistingBeneficiariesInfo() throws InterruptedException, MessagingException, IOException
	{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData("Data", 0, startIter)))
				 {
					...
					verifyText(ObjectRepository.primaryApplicantInfoPageTitle,"Primary Applicant Information");
					//Fill up applicant info page with different values
					...
					getElement(ObjectRepository.idnmbr_txt).clear();
					getElement(ObjectRepository.idnmbr_txt).sendKeys(idnumber);
					test.log(Status.INFO, "ID number entered");
					...
					getElement(ObjectRepository.crrntemplyr_txt).clear();
					getElement(ObjectRepository.crrntemplyr_txt).sendKeys(crrnt_emplyr);
					test.log(Status.INFO, "Current Employer name entered");
					
					getElement(ObjectRepository.occptn_txt).clear();
					getElement(ObjectRepository.occptn_txt).sendKeys(occupation);
					test.log(Status.INFO, "Occupation name entered");
					...
					//Validate whether pre-checked Add Joint Owner checkbox is still checked or not
					String checkedFlag = getElement("//span[@id='cbAddBene']//div").getAttribute("class");
					Assert.assertEquals(checkedFlag.contains("checked"), true);
					getElement(ObjectRepository.continue_btn).click();
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify that Non member user can modify existing Beneficiary info");
		}
	}
