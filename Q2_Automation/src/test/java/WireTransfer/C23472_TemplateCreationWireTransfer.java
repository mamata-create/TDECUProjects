...
public class C23472_TemplateCreationWireTransfer extends GenericKeywords {
	...
	@Test
	public void C23472_TemplateCreationWireTransferFromGrid() throws InterruptedException, MessagingException, IOException
	{
		...
		if(continuetestcase==true)
		{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter)))
				 {
					...
					String templateName = excl.getCellData(sheetName, 11, startIter)+randomNum; 
			...
			//		
					...
					Thread.sleep(2000);
					validateAccountInfoandSelect("//table[contains(@class,'table whitefield-form')]//following::td",displayName);
					Thread.sleep(2000);
					validateAccountInfoandSelect("//table[contains(@class,'table whitefield-form')]//following::td",displayName);
					
					getElement(ObjectRepository.cancel_btn).click();
					Thread.sleep(2000);
					getElement(ObjectRepository.newTempLateDropdown_btn).click();
					getElement(ObjectRepository.domesticWireOption).click();
					getElement(ObjectRepository.templateNameTxt).sendKeys(templateName);
					getElement(ObjectRepository.wiretrnsfr_nxtbtn).click();
					Thread.sleep(2000);
					verifyText(ObjectRepository.VisibletemplateName,templateName);
					 }
			 }
		}
