...
public class C23799_ApplicationProvideAlertForInvalidDOBEntries extends GenericKeywords  {
	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify application should alert user for invalid DOB entries
	 */
	@BeforeTest
	...
	@Test
	public void C23799_ApplicationProvideAlertForInvalidDOBEntries() throws InterruptedException, MessagingException, IOException
	{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter)))
				 {
					...
					//Entering letters in DOB field
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("October");
					
					//Entering Special Charecters in the DOB field
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("@#$%^");
					getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					Thread.sleep(500);
					verifyText(ObjectRepository.dobField_alert,"Please use a 4-digit year");
					
					
					//Entering month value greater than 12
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("13");
					Thread.sleep(500);
					//getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.dobField_alert,"Please enter a valid month");
					
					
					//Entering day in between 32 to 39
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("12/33");
					Thread.sleep(500);
					getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.dobField_alert,"Please enter a day of 31 or less");
					
					
					//Entering Invalid Year and check
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("12/30/19");
					Thread.sleep(500);
					getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.dobField_alert,"Please enter a date in the following format: mm/dd/yyyy");
					
					
					//Entering date as 01/01/1889
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("01/01/1889");
					getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.dobField_alert,"Please enter a valid date between 01/01/1890 and 12/06/2001");
					
					
					//Entering date as 12/06/2002
					getElement(ObjectRepository.dob_cal).click();
					getElement(ObjectRepository.dob_cal).clear();
					getElement(ObjectRepository.dob_cal).sendKeys("12/06/2002");
					getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.dobField_alert,"Please enter a valid date between 01/01/1890 and 12/06/2001");
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify application should alert user for invalid DOB entries");
		}
	}
