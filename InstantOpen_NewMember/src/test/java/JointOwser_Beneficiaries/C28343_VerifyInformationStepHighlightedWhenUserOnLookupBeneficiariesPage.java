...
public class C28343_VerifyInformationStepHighlightedWhenUserOnLookupBeneficiariesPage extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * As an applicant, I expect the Information step to be highlighted when I am on the Lookup Beneficiaries page.
	 */
	...
	@Test
	public void C28343_VerifyInformationStepHighlightedWhenUserOnLookupBeneficiariesPage() throws InterruptedException, MessagingException, IOException
	{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData("Data", 0, startIter)))
				 {
					...
					//Add Existing Member link
					getElement(ObjectRepository.add_existing_member_link).click();
					...
					for(WebElement eachOption :retrunElements(ObjectRepository.progressBarOptions)){
						...
						if(text_from_option.equalsIgnoreCase("Information") && class_prop.equals("JourneyBarCurrent")){
							Assert.assertTrue(true, "Information icon is not currently selected and highlighted on the page");
							return;
						}else if(text_from_option.equalsIgnoreCase("Products") && class_prop.equals("JourneyBarPrevious")){
							Assert.assertTrue(true, "Products icon is not currently selected and highlighted in different blue color");
						}else if(text_from_option.equalsIgnoreCase("Start") && class_prop.equals("JourneyBarPrevious")){
							Assert.assertTrue(true, "Start icon is not currently selected and highlighted in different blue color");
						}
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "As an applicant, I expect the Information step to be highlighted when I am on the Lookup Beneficiaries page.");
		}
	}
