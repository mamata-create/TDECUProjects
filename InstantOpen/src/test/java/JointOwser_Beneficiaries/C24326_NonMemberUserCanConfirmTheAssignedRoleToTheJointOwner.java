...
public class C24326_NonMemberUserCanConfirmTheAssignedRoleToTheJointOwner extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify that Non member user can confirm the assigned role of the Joint Owner
	 */
	...
	@Test
	public void C24326_NonMemberUserCanConfirmTheAssignedRoleToTheJointOwner() throws InterruptedException, MessagingException, IOException
	{
...
//					test.log(Status.INFO, "Current Employer name entered");
					...
					getElement(ObjectRepository.addressYearBox).sendKeys("90");
					getElement(ObjectRepository.addressMonthBox).sendKeys("2");
					getElement(ObjectRepository.currentEmp).sendKeys("TestEmp");
					getElement(ObjectRepository.currentEmp).sendKeys("TestEmp");
					getElement(ObjectRepository.monthlySal).sendKeys("4000");
					getElement(ObjectRepository.employeeDurationYear).sendKeys("99");
					getElement(ObjectRepository.employeeDurationMonth).sendKeys("3");
					...
					//Validating first column data
					String expectedProducts[] = new String[]{"Classic MasterCard ®","High Yield Checking","Share"};
					List<WebElement>allSelectedProducts = retrunElements("//table[@id='ProductRelationships_table']//td[contains(@id,'_Name')]");
					int index = 0;
					for(WebElement eachProduct:allSelectedProducts){
						String productFromPage = eachProduct.getText();
						boolean prod_flag = productFromPage.contains(expectedProducts[index].toString());
						Assert.assertEquals(prod_flag, true);
						index =  index + 1;
//						for(int index=0;index<expectedProducts.length;index++){
//							boolean prod_flag = productFromPage.contains(expectedProducts[index].toString());
//							Assert.assertEquals(prod_flag, true);
//						}
					}
					...
					List<WebElement>allSelectedCheckBoxes = retrunElements("//table[@id='ProductRelationships_table']//span[@disabled='disabled']/div");
					for(WebElement eachCheckBox:allSelectedCheckBoxes){
						String checkBoxPropertyFlagFromPage = eachCheckBox.getAttribute("class");
						boolean disabledAndCheckedFlag = checkBoxPropertyFlagFromPage.contains("checked disabled");
						if(disabledAndCheckedFlag){
					...
					List<WebElement>allEditableCheckBoxes = retrunElements("//table[@id='ProductRelationships_table']//div[@class='icheckbox_square-grey checked']");
					for(WebElement eachCheckBox:allEditableCheckBoxes){
						String checkBoxPropertyFlagFromPage = eachCheckBox.getAttribute("class");
						boolean disabledAndCheckedFlag = checkBoxPropertyFlagFromPage.contains("checked");
						if(disabledAndCheckedFlag){
							Assert.assertTrue(true);
					...
					//Validate Un-Check and Check
					getElement("(//table[@id='ProductRelationships_table']//div[@class='icheckbox_square-grey checked'])[1]").click();
					Thread.sleep(500);
					getElement("(//table[@id='ProductRelationships_table']//div[@class='icheckbox_square-grey'])[1]").click();
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, " Verify that Non member user can confirm the assigned role of the Joint Owner");
		}
	}
