...
public class VerifyUserAlreadyEnrolled extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify User already  Enroll
	 */
	...
	@Test
	public void VerifyUserAlreadyEnrolled() throws InterruptedException {
		if (continuetestcase == true) {
			...
			for (startIter = 1; startIter <= totalRowCount; startIter++) 
			{
				if (this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter))) {
					...
					if(val_msg.contains("you are already enrolled")){
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify User Already Enroll scenario working fine");
		}
	}
