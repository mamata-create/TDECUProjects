...
public class GenericKeywords extends BaseClass {
	...
	public static String getCssValue(String locator, String Attribute) throws InterruptedException {

		System.out.println("1");
		WebElement element = driver.findElement(By.xpath(locator));
		String attribute_value = element.getCssValue(Attribute);
		return attribute_value;
	...
	public static void selectDropdown_firstOption(String locator) {
		WebElement element = getElement(locator);
		Select sel = new Select(element);
		sel.selectByIndex(0);
	...
	// ashish
	public static void mouseHoverOverandClick(String locator) {
		try {
			Thread.sleep(500);
			WebElement element = getElement(locator);
			Actions action = new Actions(driver);
	...
	public static void selectFutureDate(int noofdays) throws InterruptedException {
		...
		List<WebElement> list = driver.findElements(
				By.xpath("//div[@class='calendar-days clearfix']//div[contains(@class,'cal-day is-valid')]"));
	...
	public static String fetchOutlookOTP() throws MessagingException, IOException {
		...
		Message[] messages = emailInbox.search(new FlagTerm(new Flags(Flags.Flag.SEEN), false));
	...
	public static void selectCheckboxBasedonTerm(String selectedTerm) {
		...
		for (int count = 1; count <= terms.size(); count++) {
			...
			if (termValue.contains(selectedTerm)) {
				String specificCheckbox = "(//div[contains(@style,'display: block')]//div[contains(@class,'icheckbox')])["
						+ count + "]";
	...
	// public static void invokeSikuli(String imageName, String action, String
	// value) throws FindFailed {
	//
	// System.out.println("Sikuli invoked!");
	// String filePathValue =
	// System.getProperty("user.dir")+"/imageLocator/"+imageName;
	...
	public static void checkingOptionValidation(String checkingType) {
		...
		for (int index = 1; index <= allCheckingOptions.size(); index++) {
			...
			String eachOptionLocator = "//label/strong[contains(text(),'" + checkingType + "')]/following::ul[1]/li["
					+ index + "]";
	...
	public static void serviceOptionValidation(String checkingType, String serviceOption) throws InterruptedException {
		...
		for (int count = 1; count <= services.size(); count++) {
			...
			String eachServiceOption = driver
					.findElement(By.xpath("(//div[contains(@class,'customizePanel')]//span/label)[" + count + "]"))
	...
	public void checkBoxCheckedAndUncheck(String action) {
		...
		for (int index = 1; index <= allServiceCheckBoxes.size(); index++) {
			String eachCheckboxLocator = "(//div[contains(@class,'customizePanel')]//span/div[contains(@class,'icheckbox_square-grey')])["
					+ index + "]";
	...
	public static void toolTipValidation(String modalHeaderID) throws InterruptedException {
		...
		for (int count = 1; count <= allToolTips.size(); count++) {
			String eachToolTipLocator = "(//div[contains(@class,'customizePanel')]//button[@data-toggle='modal'])["
					+ count + "]";
	...
	public static void productOptions(String productOptions) {
		...
		for (int count = 1; count <= allProductOptions.size(); count++) {
			...
			String eachProductOptionFromPage = getElement(
					"(//*[contains(text(),'Credit Cards & Loans')]//following::*[contains(@class,'panel-title')])["
							+ count + "]").getAttribute("innerText").trim();
	...
	public static void expandProductDetailsValidateandCollapse() {
		...
		for (int count = 1; count <= allArrows.size(); count++) {
			...
			String eachArrowLocator = "(//*[contains(text(),'Credit Cards & Loans')]//following::*[contains(@class,'panel-title')]/i)["
					+ count + "]";
	...
	public static void expandProductsAndValidateEachOptions(String productName, String subProductName) {
		...
		for (int index = 1; index <= allSubProducts.size(); index++) {
			...
			String eachSubProductName = driver
					.findElement(By.xpath("(//span[@id='" + subProductName + "']//label)[" + index + "]"))
			...
			if (productName.equalsIgnoreCase("Credit Cards")) {
				...
				driver.findElement(
						By.xpath("(//span[@id='CreditCard']//label/following::ul[contains(@class,' cardDescr')])["
								+ index + "]"))
						.isDisplayed();
				Assert.assertTrue(true);
			}
			if (productName.equalsIgnoreCase("Personal Loans")) {
				...
				Assert.assertEquals(checkBoxProp, "icheckbox_square-grey");
				getElement(checkBoxLocator).click();
				// driver.findElement(By.xpath("(//span[@id='CreditCard']//label/following::ul[contains(@class,'
				// cardDescr')])["+index+"]")).isDisplayed();
				// Assert.assertTrue(true);
			}
			...
			if (productName.equalsIgnoreCase("Vehicle Loans and Refinance Options ")) {
				...
				String bulletPointLocator = "(//*[contains(text(),'Vehicle Loans')]/following::div[contains(@id,'Vehicle_pnlProductMoreInfo')]//li)["
						+ index + "]";
				...
				if (Text != null && Text.equalsIgnoreCase("Check out the latest rates here")) {
					getElement(checkBoxLocator).click();
					String linkLocator = "(//*[contains(text(),'Vehicle Loans')]/following::div[contains(@id,'Vehicle_pnlProductMoreInfo')]//li)["
							+ index + "]//a";
	...
	public static boolean creditCardCheckedUncheckedValidator(String cardName, boolean check) {
		boolean flag = true;
		String creditCardLocator = "//div[contains(@id,'CreditCard_ProductPanel')]//following::label[contains(text(),'"
				+ cardName + "')]";
		if (check == true) {
			String checkedStatusLocator = "//div[contains(@id,'CreditCard_ProductPanel')]//following::label[contains(text(),'"
					+ cardName + "')]/../div[1]";
	...
	public static boolean checkamountandNavigateNextthenValidate(String cardName) {
		...
		String limitLocator = "//div[contains(@id,'CreditCard_ProductPanel')]//following::label[contains(text(),'"
				+ cardName + "')]/following::li[2]";
		String cardLimitFromPage = getElement(limitLocator).getAttribute("innerText");
		getElement(ObjectRepository.continue_btn).click();
		boolean limitChecked = getElement(ObjectRepository.creditcardLimitTxt).getAttribute("placeholder")
				.contains(cardLimitFromPage.substring(25));
		if (limitChecked)
			return status = true;
		return status;
	}
	...
	public static void validatProductOptionsDetails(String subProduct, String subProductTag) {
		...
		String subProductHeaderLocator = "//span[@id='" + subProduct + "']//following::label[contains(@for,'"
				+ subProduct + "_ProductCheckbox_')]";
		String bulletPointLocator = "(//label[contains(text(),'" + subProductTag + "')]/following::div[contains(@id,'"
				+ subProduct + "_pnlPro')])[1]//li";
		...
		String eachHeaderValueLocator = "(//span[@id='" + subProduct + "']//following::label[contains(@for,'"
				+ subProduct + "_ProductCheckbox_')])[1]";
		...
		for (int count = 1; count <= bulletElements.size(); count++) {
			String eachBulletLocator = "(//label[contains(text(),'" + subProductTag
					+ "')]/following::div[contains(@id,'" + subProduct + "_pnlPro')])[1]//li[" + count + "]";
			...
			if (bulletOptionValue.contains("latest rates here")) {
				String linkLocator = "(//label[contains(text(),'" + subProductTag + "')]/following::div[contains(@id,'"
						+ subProduct + "_pnlPro')])[1]//li[" + count + "]//a";
				getElement(linkLocator).getTagName();
				Assert.assertTrue(true);
				getElement(linkLocator).click();
				handleMultipleWindow(1, "");
	...
	public static void checkMultipleLoanOptionAndContinueOtherOption(String subProduct, String option) {
		...
		for (int index = 1; index <= allSubProducts.size(); index++) {
			String subProductNameLocator = "(//span[@id='" + subProduct + "']//following::label[contains(@for,'"
					+ subProduct + "_ProductCheckbox_')])[" + index + "]";
	...
	public static void validateVariousOptionFromPage(String action, String Option) {
		...
		if (action.equalsIgnoreCase("PanelBody")) {
			...
			for (int index = 1; index <= allOptions.size(); index++) {
				...
				String eachOptionLocator = "//*[contains(text(),'Certificates of Deposit')]/following::div[1]//li["
						+ index + "]";
			...
			for (int index = 1; index <= allOptions.size(); index++) {
				...
				String eachOptionLocator = "//*[contains(text(),'Money Market Accounts')]/following::div[1]//li["
						+ index + "]";
	...
	public static void verifyContainsValue(String locator, String text) {
		WebElement element = getElement(locator);
		String actText = element.getAttribute("value");
		System.out.println("value of contains field" + actText);
		if (actText.contains(text)) {
			Assert.assertTrue(true);
		} else {
			Assert.assertTrue(false);
	...
	public static void enteramountbasedonLimitandValidateBhaviour(int minamount, String expectedMinAmt) {
		...
		// String miniumAmountFromPage =
		// getElement(ObjectRepository.creditcardLimitTxt).getAttribute("placeholder").split("
		// ")[1];
	...
	public static void validate_txt_Field(String locator, String fieldValue,String estric,String errorMessage) throws InterruptedException {
		getElement(locator).clear();
		getElement(ObjectRepository.button_Continue).click();
		
		verifyText(ObjectRepository.specificErrorSummary,errorMessage );
		System.out.println("Error Message 1 ="+getElement(ObjectRepository.specificErrorSummary).getText());
		
		String fieldColor = getAttribute(locator, "style");
		System.out.println("fieldColor: " +fieldColor);
		
		if (fieldColor.contains("LightYellow") ||fieldColor.contains("lightyellow") ) {
			System.out.println("Field Color changed to yellow");

			verifyElementPresent(estric);
			System.out.println("* Sign is displayed");
		} else {
			System.out.println("Validation message not displayed");
		}

		getElement(locator).sendKeys(fieldValue);
		getElement(ObjectRepository.button_Continue).click();

		verifyText(ObjectRepository.header_MembershipEligibility, "Membership Eligibility");
		System.out.println("Membership Eligibility page displayed");

		getElement(ObjectRepository.button_Back).click();
		Thread.sleep(5000);

		//verifyElementNotPresent(ObjectRepository.specificErrorSummary);
		//System.out.println("Error messages not displayed for field");
	}
	
	public static void validate_txt_Field(String locator, String fieldValue,String estric,String errorMessage,String errorMessage2) throws InterruptedException {
		getElement(locator).clear();
		getElement(ObjectRepository.button_Continue).click();
		...
		verifyText(ObjectRepository.errorSummary2,errorMessage2 );
		System.out.println("Error message 2 ="+getElement(ObjectRepository.errorSummary2).getText());
	...
	public static void validate_dropdown_Field(String locator, String fieldValue,String estric,String errorMessage) throws InterruptedException {
		...
		selectDropdown_firstOption(locator);
		...
		String fieldColor = getAttribute(locator, "style");
		System.out.println("fieldColor: " +fieldColor);
		if (fieldColor.contains("yellow") || fieldColor.contains("lightyellow") ) {
			System.out.println("Field Color changed to yellow");
...
public static void validate_dropdown_dependent_Field(String locator, String fieldValue,String dependentLocator,String dependentValue,String estric,String errorMessage) throws InterruptedException {
		
		selectDropdownOpt(ObjectRepository.idtyp_drop,fieldValue);
		selectDropdown_firstOption(dependentLocator);
		Thread.sleep(2000);
		getElement(ObjectRepository.button_Continue).click();
		...
		selectDropdownOpt(dependentLocator,dependentValue);
		getElement(ObjectRepository.button_Continue).click();
