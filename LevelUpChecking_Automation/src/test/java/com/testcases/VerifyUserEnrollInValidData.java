...
public class VerifyUserEnrollInValidData extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify User  Enroll invalid data
	 */
	...
	@Test
	public void VerifyUserEnrollInValidData() throws InterruptedException {
		if (continuetestcase == true) {
			...
			for (startIter = 1; startIter <= totalRowCount; startIter++) 
			{
				if (this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter))) {
					...
					String errcode=excl.getCellData(sheetName,8, startIter);
					...
					if(val_msg.contains("verify the accuracy of the information")){
						Assert.assertTrue(true);
						test.log(Status.INFO, "Validation message displayed");
					}else{
						Assert.assertTrue(false);
					}
					
					Assert.assertTrue(val_msg.contains(errcode));
					test.log(Status.INFO, "Validation message contains the correct error code");
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify User Enroll Invalid data scenario working fine");
		}
	}
