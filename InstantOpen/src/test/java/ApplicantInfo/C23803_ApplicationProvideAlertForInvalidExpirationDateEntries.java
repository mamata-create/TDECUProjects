...
public class C23803_ApplicationProvideAlertForInvalidExpirationDateEntries extends GenericKeywords  {
	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify application should alert user for invalid Expiration Date entries
	 */
	@BeforeTest
	...
	@Test
	public void C237803_ApplicationProvideAlertForInvalidExpirationDateEntries() throws InterruptedException, MessagingException, IOException
	{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter)))
				 {
					...
					//Entering letters in Expiration Date field
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("October");
					
					//Entering Special Charecters in the Expiration Date field
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("@#$%^");
					getElement(ObjectRepository.expdt_cal).sendKeys(Keys.TAB);
					Thread.sleep(500);
					verifyText(ObjectRepository.expirationdate_alert,"Please use a 4-digit year");
					
					
					//Entering month value greater than 12
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("13");
					Thread.sleep(500);
					//getElement(ObjectRepository.dob_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.expirationdate_alert,"Please enter a valid month");
					
					
					//Entering day in between 32 to 39
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("12/33");
					Thread.sleep(500);
					getElement(ObjectRepository.expdt_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.expirationdate_alert,"Please enter a day of 31 or less");
					
					
					//Entering Invalid Year and check
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("12/30/19");
					Thread.sleep(500);
					getElement(ObjectRepository.expdt_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.expirationdate_alert,"Please enter a date in the following format: mm/dd/yyyy");
					
					
					//Entering date as 01/01/1889
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("01/01/1889");
					getElement(ObjectRepository.expdt_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.expirationdate_alert,"Please enter a valid date between 12/06/2019 and 01/01/2200");
					
					
					//Entering date as 01/01/2201
					getElement(ObjectRepository.expdt_cal).click();
					getElement(ObjectRepository.expdt_cal).clear();
					getElement(ObjectRepository.expdt_cal).sendKeys("01/01/2201");
					getElement(ObjectRepository.expdt_cal).sendKeys(Keys.TAB);
					verifyText(ObjectRepository.expirationdate_alert,"Please enter a valid date between 12/06/2019 and 01/01/2200");
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify application should alert user for invalid Expiration Date entries");
		}
	}
