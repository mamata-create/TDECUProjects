...
public class C23966_NonMemberUserCanDeleteExistingBeneficiarydetails extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify that Non member user can Delete existing Beneficiary Details
	 */
	...
	@Test
	public void C23966_NonMemberUserCanDeleteExistingBeneficiarydetails() throws InterruptedException, MessagingException, IOException
	{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData("Data", 0, startIter)))
				 {
					...
					getElement(ObjectRepository.fname_txt).sendKeys("Nick");
					test.log(Status.INFO, "First name entered");
										
					getElement(ObjectRepository.lname_txt).sendKeys("Jonas");
					test.log(Status.INFO, "Last name entered");
					...
					//Validating added Beneficiary name and procced to Remove
					verifyText(ObjectRepository.addedUserName,"Nick Jonas");
					getElement(ObjectRepository.addedUser_removeLink).click();
					verifyText(ObjectRepository.additional_beneficiaryPageTitle,"Your Beneficiary Information");
					
					//Validating that existing Beneficiary name no longer available
					Assert.assertEquals(!ObjectRepository.addedUserName.contains("Nick Jonas"), true);
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify that Non member user can Delete existing Beneficiary Details");
		}
	}
