...
public class C23713_VerifyMemberToMemberInsufficientFundTransfer   extends GenericKeywords {
	...
	@Test
	public void C23713_VerifyMemberToMemberInsufficientFundTransfer() throws InterruptedException
	{
	...
	//Fill mandatory fields
		...
		String frmacnt_text=getElement("//android.widget.CheckedTextView[contains(@text,'"+frmacnt+"')]").getText();
		String[] text=frmacnt_text.split("$");
		//String[] newtext=text[0].split(" ");
		frmacnt_text=frmacnt_text.substring(frmacnt_text.indexOf("$"), frmacnt_text.length());
		
		frmacnt_text=frmacnt_text.replace(",", "").replace("$", "");
		double d=Double.parseDouble(frmacnt_text);
		d=d+100;
		amnt=Double.toString(d);
		System.out.println("amount to be entered-"+amnt);
		...
		verifyElementPresent(ObjectRepository.mtm_insfnd);
		test.log(Status.INFO, "MTM single transfer with insufficient fund validation Message appeared");
		
		clickElement(ObjectRepository.mtm_clsbtn);
	...
	public void afterMethod(ITestResult result) throws Throwable{
		...
		{
			test.log(Status.PASS, "Verify Member to Member Single Transfer with insufficient fund scenario working fine");
		}
	}
