...
public class C23429_VerifyStopPaymentPayeeField extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify Stop Payment Payee Field
	 */
	...
	@Test
	public void C23429_VerifyStopPaymentPayeeField() throws InterruptedException, MessagingException, IOException
	{
			...
			for(startIter=1;startIter<=totalRowCount;startIter++) //It is mandatory to have this for loop in every test case
			 {	
				...
				//Verify Stop Payment page title
					...
					getElement(ObjectRepository.stppaymnt_payeetab).click();
					test.log(Status.INFO, "Payee tab clicked");
					Thread.sleep(3000);
					
					getElement(ObjectRepository.stppaymnt_payeetxt).sendKeys(payee);
					test.log(Status.INFO, "Payee name entered");
					Thread.sleep(3000);
					
					String payee_txt=getElement(ObjectRepository.stppaymnt_payeetxt).getAttribute("value");
					System.out.println("Payee text length-"+payee_txt.length());
					Assert.assertTrue(payee_txt.length()==40);
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify Stop payments Payee Field scenario working fine");
		}
	}
