...
public class C23830_VerifyMembershipEligibilityEmployeeOptionSelection extends GenericKeywords{
  ...
  @Test
  public void C23830_VerifyMembershipEligibilityEmployeeOptionSelection() throws InterruptedException, MessagingException, IOException {
	  if(continuetestcase==true)
	  {
			...
			{	
				...
				//Enter applicant information
					...
					verifyElementPresent(ObjectRepository.cmpnylist_lnk);
					test.log(Status.INFO, "View List of companies link appearing under employee option");
					
					verifyElementPresent(ObjectRepository.emplyr_txt);
					test.log(Status.INFO, "Employer text box appearing");
					
					getElement(ObjectRepository.productPageNext).click();
					test.log(Status.INFO, "Continue button clicked from membership eligibility page");
					
					verifyText(ObjectRepository.mmbr_error,"Please select the employer");
					test.log(Status.INFO, "Error message appears for not entering employer name ");
					
			//     Enter invalid company name and click Contiue
					getElement(ObjectRepository.emplyr_txt).sendKeys("TDECU does NOT serve");
					test.log(Status.INFO, "Invalid Employer name entered");
					
					getElement(ObjectRepository.productPageNext).click();
					test.log(Status.INFO, "Continue button clicked from membership eligibility page");
					
					verifyText(ObjectRepository.mmbr_error,"The value entered is not valid. Please try again");
					test.log(Status.INFO, "Error message appears for entering invalid employer name");
					
					
			// click - view list of companies link
					String parentWindowHandle=driver.getWindowHandle();
					getElement(ObjectRepository.cmpnylist_lnk).click();
					test.log(Status.INFO, "View List of companies link clicked under employee option");
					...
					Set<String> windowHandles=driver.getWindowHandles();
					 for(String handle: windowHandles){
				            ...
				            verifyElementPresent(ObjectRepository.cmpnypage_ttl);
							test.log(Status.INFO, "Companies list page title appearing correctly");
						    
				            System.out.println("Closing the new window...");
				            driver.close();
				            test.log(Status.INFO, "Companies list window closed");
				            }
				        }
					 
				        driver.switchTo().window(parentWindowHandle);
				        test.log(Status.INFO, "Switched back to tdecu page");
  ...
  public void afterMethod(ITestResult result) throws Throwable {
	  if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify Membership elgibility - Employee option selection for TDECU account");
		}
	}
