...
public class C23860_NonMemberUserWouldLikeToModifyAccountInfoBeforeConfirming extends GenericKeywords  {
	ExtentReports extent;
	ExtentTest test;
	/*
	 * Verify Non member user can modify account info before confirming
	 */
	@BeforeTest
	...
	@Test
	public void C23860_NonMemberUserWouldLikeToModifyAccountInfoBeforeConfirming() throws InterruptedException, MessagingException, IOException
	{
...
//					test.log(Status.INFO, "Name suffix entered");
					...
					//Modify other option and continue
					getElement(ObjectRepository.getTermCertificateLocator(("6"))).click();
					getElement(ObjectRepository.CD_amount_field).sendKeys("1000");
					selectDropdownOpt(ObjectRepository.CD_term_field,"12");
					getElement(ObjectRepository.getTermCertificateLocator(("12"))).click();
					getElement(ObjectRepository.continue_btn).click();
					verifyText(ObjectRepository.primaryApplicantInfoPageTitle,"Primary Applicant Information");
					getElement(ObjectRepository.continue_btn).click();
					verifyElementPresent(ObjectRepository.memberShipEligibilityPage);
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify Non member user can modify account info before confirming");
		}
	}
