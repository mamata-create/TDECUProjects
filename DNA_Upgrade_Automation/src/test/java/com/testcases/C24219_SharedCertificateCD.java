...
public class C24219_SharedCertificateCD extends GenericKeywords {
			...
			for(startIter=1;startIter<=totalRowCount;startIter++) //It is mandatory to have this for loop in every test case
			 {	
				if(this.getClass().getSimpleName().equals(excl.getCellData("Data", 0, startIter)))
				 {
					...
					String amount = excl.getCellData(sheetName, 21, startIter);
					String termValue = excl.getCellData(sheetName, 22, startIter);
				...
				//Lets Get Started page	
					scrollToElement(ObjectRepository.certificateOfDeposit);
					getElement(ObjectRepository.certificateOfDeposit).click();
					test.log(Status.INFO, "Certificates of Deposit is expanded");
					scrollToElement(ObjectRepository.enterAmountTxtField);
					getElement(ObjectRepository.enterAmountTxtField).sendKeys(amount);
					selectDropdownOpt(ObjectRepository.selectTermDropdown,termValue);
					selectCheckboxBasedonTerm(termValue);
					test.log(Status.INFO, "Term is selected");
