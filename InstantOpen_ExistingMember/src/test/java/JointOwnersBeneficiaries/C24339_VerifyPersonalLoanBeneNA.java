...
public class C24339_VerifyPersonalLoanBeneNA extends GenericKeywords{
  ...
  @Test
  public void C24339_VerifyPersonalLoanBeneNA() throws InterruptedException, MessagingException, IOException {
	  if(continuetestcase==true)
	  {
			...
			{	
				if(this.getClass().getSimpleName().equals(excl.getCellData("ProdData", 0, startIter)))
				{
					...
					verifyElementNotPresent(ObjectRepository.addBeneCheckBox);
				}
			}
	  }
  }
  ...
  public void afterMethod(ITestResult result) throws Throwable {
	  if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify beneficiary option not avaialable when only personal loan is selected for existing member");
		}
	}
