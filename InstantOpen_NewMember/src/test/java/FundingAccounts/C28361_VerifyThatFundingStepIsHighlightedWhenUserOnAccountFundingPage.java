...
public class C28361_VerifyThatFundingStepIsHighlightedWhenUserOnAccountFundingPage  extends GenericKeywords{
  ...
  @Test
  public void C28361_VerifyThatFundingStepIsHighlightedWhenUserOnAccountFundingPage() throws InterruptedException, MessagingException, IOException {
			  ...
			  //Validate Disclouser tab is highlighted
					...
					for(WebElement eachOption :retrunElements(ObjectRepository.progressBarOptions)){
						...
						if(text_from_option.equalsIgnoreCase("Funding") && class_prop.equals("JourneyBarCurrent")){
							Assert.assertTrue(true, "Funding icon is not highlighted in Bule");
							return;
						}else if(text_from_option.equalsIgnoreCase("Disclosures") && class_prop.equals("JourneyBarPrevious")){
							Assert.assertTrue(true, "Disclosures icon is not highlighted as Blue");
						}
						index++;
					}
...
//					test.log(Status.INFO, "Submit Application information appearing correctly");
  ...
  public void afterMethod(ITestResult result) throws Throwable {
	  if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify My Accounts Additional Funds Transfer Option scenario working fine");
		}
	}
