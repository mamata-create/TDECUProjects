...
public class TDB2088_VerifyModifyScheduledTransaction extends GenericKeywords {
	
	ExtentReports extent;
	ExtentTest test;
/*
 * TDB-2088 - Verify modify scheduled transaction
 */
	...
	@Test
	public void TDB2088_VerifyModifyScheduledTransaction() throws InterruptedException{
	...
	//Verify Posted tab on clicking transfer activity	
		...
		Thread.sleep(4000);
		getElement(ObjectRepository.mdfytrnsctnlnk).click();
		test.log(Status.INFO, "Modify transaction link clicked");
		Thread.sleep(2000);
		
		getElement(ObjectRepository.amount_txt).clear();
		test.log(Status.INFO, "Amount field cleared");
		
		amnt=randomAmount();
		
		getElement(ObjectRepository.amount_txt).sendKeys(amnt);
		test.log(Status.INFO, "Updated amount entered");
		Thread.sleep(2000);
		
		getElement(ObjectRepository.maketransfer_btn).click();
		test.log(Status.INFO, "Update button clicked");
		Thread.sleep(2000);
		
		getElement(ObjectRepository.cnfrmcnclbtn).click();
		test.log(Status.INFO, "Cancel button clicked from transfer details screen");
		Thread.sleep(2000);
		
		getElement(ObjectRepository.cnfrmnobtn).click();
		test.log(Status.INFO, "No button clicked");
		Thread.sleep(2000);
...
public void afterMethod(ITestResult result) throws Throwable{
	...
	{
		test.log(Status.PASS, "Verify modify scheduled transaction scenario working fine");
	}
}
