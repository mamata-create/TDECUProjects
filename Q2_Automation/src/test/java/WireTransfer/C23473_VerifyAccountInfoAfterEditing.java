...
public class C23473_VerifyAccountInfoAfterEditing extends GenericKeywords {
	...
	@Test
	public void C23473_VerifyAccountInfoAfterEditingfromGrid() throws InterruptedException, MessagingException, IOException
	{
		...
		if(continuetestcase==true)
		{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData(sheetName, 0, startIter)))
				 {
					...
					String modifiedWireName = excl.getCellData(sheetName, 10, startIter)+randomNum; 
			...
			//		
					...
					//Steps to validate account info after editing
					
					getElement(ObjectRepository.listvw_btn).click();
					test.log(Status.INFO, "List View button clicked");
					Thread.sleep(2000);
					validateAccountInfoFromGrid("//table[contains(@class,'table whitefield-form')]//following::td",displayName);
					Thread.sleep(2000);
					getElement(ObjectRepository.wiretrnsfr_wrnm).click();
					getElement(ObjectRepository.wiretrnsfr_wrnm).clear();
					getElement(ObjectRepository.wiretrnsfr_wrnm).sendKeys(modifiedWireName);
					test.log(Status.INFO, "Wire Name Edited");
					
					scrollToElement(ObjectRepository.wiretrnsfr_updtrcp);
					getElement(ObjectRepository.wiretrnsfr_updtrcp).click();
					test.log(Status.INFO, "Updated recipent details saved");
					Thread.sleep(4000);
					
					validateAccountInfoFromGrid("//table[contains(@class,'table whitefield-form')]//following::td",displayName);
					Thread.sleep(2000);
					
					//verifyText(ObjectRepository.wiretrnsfr_wrnm,modifiedWireName);
