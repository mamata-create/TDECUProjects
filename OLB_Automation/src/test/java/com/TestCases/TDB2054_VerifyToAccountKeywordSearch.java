...
public class TDB2054_VerifyToAccountKeywordSearch extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * TDB2054 - Verify From Account Keyword Search
	 */
	...
	@Test
				public void TDB2054_VerifyToAccountKeywordSearch() throws InterruptedException
			{
					if(continuetestcase==true)
					{
						...
						for(startIter=1;startIter<=totalRowCount;startIter++) //It is mandatory to have this for loop in every test case
						 {	
							...
							//Search To Account	
								scrollToElement(ObjectRepository.amount_txt);
								Thread.sleep(3000);
								verifyElementPresent(ObjectRepository.toacntdroparw);
								getElement(ObjectRepository.toacntdroparw).click();
								test.log(Status.INFO, "To account dropdown opened" );
								getElement(ObjectRepository.tosearchtxt).sendKeys(toAcnt);
								getElement(ObjectRepository.tosearchImg).click();
								test.log(Status.INFO, "Search icon clicked after entering account number" );
								Thread.sleep(3000);
								
								String searched=getElement(ObjectRepository.tosearched).getText();
								System.out.println("Searched text-"+searched);
								Assert.assertTrue(searched.contains(".."+toAcnt));
								test.log(Status.INFO, "To account searched correctly" );
								
								getElement(ObjectRepository.toclearImg).click();
								test.log(Status.INFO, "Clear icon clicked" );
								String text=getElement(ObjectRepository.tosearchtxt).getText();
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify To Account Keyword Search scenario working fine");
		}
	}
