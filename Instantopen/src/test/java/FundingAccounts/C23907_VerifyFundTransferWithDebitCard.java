...
public class C23907_VerifyFundTransferWithDebitCard  extends GenericKeywords{
  ...
  @Test
  public void C23907_VerifyFundTransferWithDebitCard() throws InterruptedException, MessagingException, IOException {
	  if(continuetestcase==true)
	  {
			...
			//Membership Eligibility page		
				...
				//Account Funding
					...
					getElement(ObjectRepository.dbtcreditcrd_opt).click();
					test.log(Status.INFO, "Debit card option selected");
					
					getElement(ObjectRepository.crdnmbr_txt).sendKeys("600712349876");
					test.log(Status.INFO, "Debit card number entered");
					
					selectDropdownOpt(ObjectRepository.crdmonth_drop, "jan");
					test.log(Status.INFO, "Card expiry month selected");
					
					selectDropdownOpt(ObjectRepository.crdyr_drop, "2026");
					test.log(Status.INFO, "Card expiry year selected");
					
					getElement(ObjectRepository.crdcvv_txt).sendKeys("576");
					test.log(Status.INFO, "CVV number entered");
					
					getElement(ObjectRepository.crdfrstname_txt).sendKeys("John");
					test.log(Status.INFO, "Card Holder first name entered");
					
					getElement(ObjectRepository.crdlstname_txt).sendKeys("Wright");
					test.log(Status.INFO, "Card Holder last name entered");
					...
					verifyElementPresent("//*[contains(text(),'Thank You for Your Application')]");
					test.log(Status.INFO, "Application submittion success message appearing");
  ...
  public void afterMethod(ITestResult result) throws Throwable {
	  if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "Verify fund transfer with debit card scenario working fine");
		}
	}
