...
public class C28366_VerifyThankYouPageComponents extends GenericKeywords {

	ExtentReports extent;
	ExtentTest test;
	/*
	 * As an authenticated Member, I want the Thank You page to provide clear instructions on next steps.
	 */
	...
	@Test
	public void C28366_VerifyThankYouPageComponents() throws InterruptedException, MessagingException, IOException
	{
				...
				if(this.getClass().getSimpleName().equals(excl.getCellData("Data", 0, startIter)))
				 {
					...
					String promo_code = excl.getCellData(sheetName, 30, startIter);
					String title = "We’re already heads down, reviewing your application. We’re pretty quick so expect to hear from us within one business day — we’ll send a response to the email address we have on file.";
					String upload_document = "For security purposes, we may need additional documentation to complete your application. You can even take pictures of the requested documents with your smartphone and upload them below. You can also upload the documents at a later time by visiting the Application Status Center.";
...
//					test.log(Status.INFO, "Name suffix entered");
			...
			// Terms and conditions page	
				...
				//Identity verification page
					...
					verifyText(ObjectRepository.text_on_thank_you_page("1"),title);
					verifyText(ObjectRepository.text_on_thank_you_page("2"),upload_document);
					verifyText(ObjectRepository.text_on_thank_you_page("3"),"And as much as we love selfies, uploading those will delay processing times.");
					
					//Applicant record
					
					for(WebElement eachRecord : retrunElements(ObjectRepository.dataTable("1"))){
						
						if(!eachRecord.getText().isEmpty()){
							Assert.assertTrue(true, "Record found empty!");
						}
					}
					
					//Uploading document section
					for(WebElement eachRecord : retrunElements(ObjectRepository.dataTable("2"))){
	...
	public void afterMethod(ITestResult result) throws Throwable {
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.log(Status.PASS, "As an authenticated Member, I want the Thank You page to provide clear instructions on next steps.");
		}
	}
